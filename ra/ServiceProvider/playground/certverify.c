...
int main() {
  ...
  int outlength;
  char *cres = curl_easy_unescape(curl, uri_cert_buf, sizeof(uri_cert_buf), &outlength);
   ...
   * ---------------------------------------------------------- */
    certbio = BIO_new_mem_buf(res.c_str(), res.length());
