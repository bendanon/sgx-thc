...
static const char* skg_mrenclave = "h39gowM7F0Au0m6JfC1DoMO6K1GImzd3MXHK9HAwsP8=";
...
sgx_status_t verify_skg(verification_report_t* p_report, sgx_ec256_public_t* peer_pk){
    ...
    if(0!=memcmp(skg_mrenclave, 
                 base64_encode((uint8_t*)&quote_body.report_body.mr_enclave, sizeof(quote_body.report_body.mr_enclave)).c_str(),
                 strlen(skg_mrenclave)))
    {
        ocall_print("mrenclave is invalid %d", 0);
        return SGX_ERROR_UNEXPECTED;
    }
...
sgx_status_t bb_init_1(sgx_sealed_data_t* p_sealed_data, size_t sealed_size, 
    ...
    status = verify_skg(p_report, p_skg_pk);

    if(status){
        ocall_print("verify_skg failed");
        return status;
    }
