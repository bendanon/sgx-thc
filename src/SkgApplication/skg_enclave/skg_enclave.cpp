...
#include "../common_enclave/Base64_enclave.h"
...
sgx_status_t verify_bb(verification_report_t* p_report, sgx_ec256_public_t* peer_pk, sgx_measurement_t* peer_mrenclave){

    if(SGX_SUCCESS != verify_report(p_report)){
        ocall_print("verify_report failed");
    ...
    {
        ocall_print("mrsigner is invalid %d", 0);
        return SGX_ERROR_UNEXPECTED;
    }

    memcpy(peer_mrenclave, &quote_body.report_body.mr_enclave, sizeof(peer_mrenclave));
...
sgx_status_t skg_exec(sgx_ec256_public_t* p_bb_pk, sgx_ec256_public_t* p_skg_pk, size_t pk_size,  //in (bbpk, pk)
                      ...
                      uint8_t* s_skg_encrypted, size_t s_skg_encrypted_size)         //out (c')
{
    ...
    sgx_measurement_t peer_mrenclave;
    status = verify_bb(p_report, p_bb_pk, &peer_mrenclave);

    if(status){
        ocall_print("verify_bb failed");
        return status;
    }
    ...
    uint8_t secret_base[sizeof(sgx_measurement_t) + SECRET_KEY_SIZE_BYTES];
    memcpy(secret_base, s_sk_unsealed, SECRET_KEY_SIZE_BYTES);
    memcpy(secret_base + SECRET_KEY_SIZE_BYTES, &peer_mrenclave, sizeof(peer_mrenclave));
    sgx_sha256_hash_t s_skg;

    status = sgx_sha256_msg(secret_base, sizeof(secret_base), &s_skg);

    if(status) {
        ocall_print("sgx_sha256_msg status is %d\n", status);
        return status;
    } 

    status = encrypt(s_skg, sizeof(s_skg), s_skg_encrypted, (uint8_t*)&shared_key);
